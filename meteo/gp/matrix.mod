...
system("cat ./tmp/${filename} | awk -F';' '{if (last_key != $1) {print \"\"} print $0; last_key = $1}' | tr '[;]' ' ' > ./tmp/meteo_blank.csv")

set title 'Stations ${V2} map' font ',16'
...
splot './tmp/meteo_blank.csv' using 2:3:${V5}  with pm3d title '${V2} Map'
