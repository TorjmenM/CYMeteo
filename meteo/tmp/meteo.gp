...
plot './tmp/g_s_t1_meteo.csv' using 0:3:2:xtic(1) with filledcurve fc rgb Shadecolor title "Shadowed variation region",'' using 0:4 smooth mcspline lw 1 lc rgb '#80E0A080' title "Average Temperature"
reset
#!/usr/local/bin/gnuplot -persist
...
set grid
set format x '%Y/%m/%d'
system("awk -F ';' '{ if ((a != substr($1,1,7)) && (NR!=1)) { print \"\";} a=substr($1,1,7); print substr($1,1,2),substr($1,3,5),substr($1,8,8),$2 }' ./tmp/g_s_t3_meteo.csv > ./tmp/data.csv") 
set datafile separator " "
set palette defined (0 "blue",2 "green",4 "yellow",6 "orange",8 "red")
set cbrange [0:23]
plot './tmp/data.csv' using 3:4:1 with lines lt palette title "Temperature - Colour per hour"
reset
#!/usr/local/bin/gnuplot -persist
...
set grid
set format x '%Y/%m/%d'
system("awk -F ';' '{ if ((a != substr($1,1,7)) && (NR!=1)) { print \"\";} a=substr($1,1,7); print substr($1,1,2),substr($1,3,5),substr($1,8,8),$2 }' ./tmp/g_s_p3_meteo.csv > ./tmp/data.csv") 
set datafile separator " "
set palette defined (0 "blue",2 "green",4 "yellow",6 "orange",8 "red")
set cbrange [0:23]
plot './tmp/data.csv' using 3:4:1 with lines lt palette title "Pressure - Colour per hour"
reset
#!/usr/local/bin/gnuplot -persist
...
system("cat ./tmp/g_s_m_meteo.csv | awk -F';' '{if (last_key != $1) {print \"\"} print $0; last_key = $1}' | tr '[;]' ' ' > ./tmp/meteo_blank.csv")
...
stats './tmp/g_s_w_meteo.csv' u 3:2 nooutput
...
plot 	'./tmp/g_s_w_meteo.csv' using 3:2:(column(5)*scale/50):(column(4)*scale/50):(sqrt(column(4)**2 + column(5)**2)) \
							with vectors filled head lw 1 lc palette title 'average wind direction and amplitude', \
		'./tmp/g_s_w_meteo.csv'	using 3:2:(Wind(4,5)) with labels hypertext point pt 6 ps 0.6 lt -1 notitle,\
